...
public class Cart_Adapter  extends RecyclerView.Adapter<Cart_Adapter.ViewHolder> {
    ...
    public Cart_Adapter(List<Items> itemsList, ItemRemoved itemRemoved){
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, @SuppressLint("RecyclerView") int position) {
        ...
        holder.cartPrice.setText("PKR "+itemsList.get(position).getPrice());
        Glide.with(holder.itemView.getContext()).load(itemsList.get(position).getImg_url()).into(holder.cartImage);
        holder.removeItem.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                firebaseFirestore.collection("User-Cart items").document(firebaseAuth.getCurrentUser().getUid())
